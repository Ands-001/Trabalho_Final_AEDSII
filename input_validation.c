...
int lerInteiroPositivo(const char *mensagem) {
    int valor;
    int result;
    char linha[100]; // Buffer para ler a linha inteira

    do {
        printf("%s", mensagem);
        if (fgets(linha, sizeof(linha), stdin) != NULL) { // Lê a linha inteira
            result = sscanf(linha, "%d", &valor); // Tenta ler o inteiro da linha

            // Verifica se a leitura foi bem-sucedida e se há caracteres extras na linha
            // (além do número e do newline, que sscanf consome)
            if (result == 1 && valor >= 1) {
                char *ptr = linha;
                while (*ptr != '\0' && (*ptr == ' ' || *ptr == '\t' || *ptr == '\n')) { // Ignora espaços e tabs iniciais
                    ptr++;
                }
                ...
                result = 0; // Marca como falha na leitura se sscanf não leu 1 item ou valor < 1
            }
        }

        if (result != 1 || valor < 1) {
            printf("Por favor, insira um número inteiro positivo válido.\n");
        }
    } while (result != 1 || valor < 1);
